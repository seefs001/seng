...
func (cors *cors) applyCors(c *seng.Context) {
	...
	if c.Request.Method == "OPTIONS" {
		...
		defer c.Status(http.StatusNoContent) // Using 204 is better than 200 when the request status is OPTIONS
	} else {
		cors.handleNormal(c)
	}
...
func (cors *cors) handlePreflight(c *seng.Context) {
...
func (cors *cors) handleNormal(c *seng.Context) {
	header := c.Writer.Header()
